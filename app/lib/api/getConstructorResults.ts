...
export const getConstructorResults = async (
  year: string,
  constructorId: string
): Promise<RawConstructorResults | null> => {
  try {
    ...
    const cacheKey = generateCacheKey(
      `constructor-results-${constructorId}`,
      year
    );
    ...
    throw new ApiError(
      `Unexpected error fetching constructor results for ${year} for constructor ${constructorId}: ${
